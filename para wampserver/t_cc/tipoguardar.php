...
if ($tipoguardar == 'consultar_turnosver31') {
	...
	$RsllAMADOTV = $mysqli->query($query_RsllAMADOTV);
	if (!$RsllAMADOTV) {
		echo "Error: " . $mysqli->error;
		exit();
	}

	$row_RsllAMADOTV = $RsllAMADOTV->fetch_assoc();
	$totalRows_RsllAMADOTV = $RsllAMADOTV->num_rows;
	...
	$RsllAMADOTV2 = $mysqli->query($query_RsllAMADOTV2);
	if (!$RsllAMADOTV2) {
		echo "Error: " . $mysqli->error;
		exit();
	}

	$totalRows_RsllAMADOTV2 = $RsllAMADOTV2->num_rows;
...
if ($tipoguardar == 'consultar_turnosreload') {
	...
	$query_RsllAMADOTV = $query_RsllAMADOTV . " ORDER BY TURNCONS asc";
	$RsllAMADOTV = mysqli_query($turnos, $query_RsllAMADOTV) or die(mysqli_error($turnos));
...
$query_RsllAMADOTV2 = "SELECT  TURNCONS CONSECUTIVO
		  ...
		  WHERE   TURNIDES = 3
			  AND TURNCONS NOT IN (" . $turnos_pantallatv . ")";
$RsllAMADOTV2 = mysqli_query($turnos, $query_RsllAMADOTV2) or die(mysqli_error($turnos));
	   ...
	   if ($tipoguardar == 'ComprobarTurnoPendiente') {
		$query_RsConsultaTurno = "SELECT 
								 ...
								 WHERE TURNIDUS = '" . $_SESSION["IDUSU"] . "'
									 AND TURNIDES IN ('3','6')";
		$RsConsultaTurno = mysqli_query($turnos, $query_RsConsultaTurno) or die(mysqli_error($turnos));
...
if ($tipoguardar == 'Sincronizar_Turno') {
	...
	//echo($query_RsInsert);											   
	$RsInsert = mysqli_query($turnos, $query_RsInsert) or die(mysqli_error($turnos));

	$query_RsUltInsert = "SELECT LAST_INSERT_ID() DATO";
	$RsUltInsert = mysqli_query($turnos, $query_RsUltInsert) or die(mysqli_error($turnos));
	...
	$RsParametro = mysqli_query($turnos, $query_RsParametro) or die(mysqli_error($turnos));
	...
	if ($_GET['modulo'] != '') {
		...
		$RsUpdate = mysqli_query($turnos, $query_RsUpdate) or die(mysqli_error($turnos));
...
if ($tipoguardar == 'Crear_TurnoAyuda') {
	...
	if ($totalRows_RsConsultaTurno > 0) {
		...
		$RsRangosTurno = mysqli_query($turnos, $query_RsRangosTurno) or die(mysqli_error($turnos));
...
if ($tipoguardar == 'Crear_turno') {
	...
	if ($secuencia == 'uno') {
		...
		if (isset($totalRows_RsConsultaTurno) && $totalRows_RsConsultaTurno > 0) {
			...
			$RsLLamarTurno = mysqli_query($turnos, $query_RsLLamarTurno) or die(mysqli_error($turnos));

			$query_RsLLamarTurno2 = "UPDATE turnos SET TURNFELL = SYSDATE() WHERE TURNCONS ='" . $row_RsConsultaTurno['CONSECUTIVO'] . "'";
			$RsLLamarTurno2 = mysqli_query( $turnos, $query_RsLLamarTurno2) or die(mysqli_error( $turnos));
...
if ($tipoguardar == 'Siguiente_turno') {
	...
	if ($secuencia_N == 'dos' or $secuencia_N == 'uno') {
		...
		$RsTurno = mysqli_query($query_RsTurno, $turnos) or die(mysqli_error());
		...
		$RsConsultaTurnoSig = mysqli_query($query_RsConsultaTurnoSig, $turnos) or die(mysqli_error());
...
if ($tipoguardar == 'Inicio_AtencionAyuda') {
	...
	$RsI_Turno = mysqli_query($query_RsI_Turno, $turnos) or die(mysqli_error());
	...
	$query_RsI_Turno2 = "UPDATE turnos 
                      ...
                      WHERE TURNCONS ='" . $_GET['consecutivo_ayuda'] . "'
                      AND   TURNPARA ='" . $_GET['parametro_ayuda'] . "' ";
	$RsI_Turno2 = mysqli_query($query_RsI_Turno2, $turnos) or die(mysqli_error());
...
if ($tipoguardar == 'Fin_AtencionAyuda') {
	...
	$query_RsF_Turno = "UPDATE turnos 
                      ...
                      WHERE TURNCONS ='" . $_GET['consecutivo_ayuda'] . "'
                      AND   TURNPARA ='" . $_GET['parametro_ayuda'] . "' ";
	$RsF_Turno = mysqli_query($query_RsF_Turno, $turnos) or die(mysqli_error());
	...
	$query_RsF_Turno2 = "UPDATE turnos 
                      ...
                      WHERE TURNCONS ='" . $_GET['consecutivo_ayuda'] . "'
                      AND   TURNPARA ='" . $_GET['parametro_ayuda'] . "' ";
	$RsF_Turno2 = mysqli_query($query_RsF_Turno2, $turnos) or die(mysqli_error());
	/*    
  ...
  $RsConsultar_Tur = mysqli_query($query_RsConsultar_Tur, $turnos) or die(mysqli_error());
...
$query_RsConsultar_Tur="SELECT `TURNCONS`,
	...
	$RsRangosTurno = mysqli_query($query_RsRangosTurno, $turnos) or die(mysqli_error());
	$row_RsRangosTurno = mysqli_fetch_assoc($RsRangosTurno);
	$totalRows_RsRangosTurno = mysqli_num_rows($RsRangosTurno);
...
if ($tipoguardar == 'Salir_Turno') {
	...
	$RsModulo = mysqli_query($query_RsModulo, $turnos) or die(mysqli_error());

	$query_RsActualizarUsuario = "
		   UPDATE usuarios SET USUAESTA = '0' WHERE USUAID ='" . $S_idusuario . "'";
	$RsActualizarUsuario = mysqli_query($query_RsActualizarUsuario, $turnos) or die(mysqli_error());

	$query_RsSalida = "UPDATE turnos 
                      SET TURNIDES = '4'
                      WHERE TURNCONS ='" . $_GET['S_consecutivo'] . "'
                      AND   TURNPARA ='" . $_GET['S_parametro'] . "' ";
	$RsSalida = mysqli_query($query_RsSalida, $turnos) or die(mysqli_error());
	...
	*/
	...
	$RsComprobarEstados = mysqli_query($query_RsComprobarEstados, $turnos) or die(mysqli_error());
	...
	if ($totalRows_RsComprobarEstados == 0) {
		$query_RsTurnosExistentes = "SELECT   T.TURNCONS,
							...
							FROM turnos T
					    WHERE TURNIDUS = '" . $_SESSION["IDUSU"] . "'";
		$RsTurnosExistentes = mysqli_query($query_RsTurnosExistentes, $turnos) or die(mysqli_error());
		...
		if ($totalRows_RsTurnosExistentes > 0) {
			do {
				...
				$RsDelete = mysqli_query($query_RsDelete, $turnos) or die(mysqli_error());
			} while ($row_RsTurnosExistentes = mysqli_fetch_assoc($RsTurnosExistentes));

			$query_RsTruncarTable = "SELECT count(TURNCONS) CANTIDAD 
		                              FROM TURNOS 
                               ";
			$RsTruncarTable = mysqli_query($query_RsTruncarTable, $turnos) or die(mysqli_error());
			...
			if ($row_RsTruncarTable['CANTIDAD'] == 0) {
				...
				$RsTruncar = mysqli_query($query_RsTruncar, $turnos) or die(mysqli_error());
			}
...
if ($tipoguardar == 'Contar_Turno') {
	...
	$RsContadorMax = mysqli_query($query_RsContadorMax, $turnos) or die(mysqli_error());
...
if ($tipoguardar == 'Ayuda_Turno') {
	...
	$RsTurnoMax = mysqli_query($query_RsTurnoMax, $turnos) or die(mysqli_error());
